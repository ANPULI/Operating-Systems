...
typedef struct task {
    ...
    unsigned int IOResponseTime;  // time needed for IO response
    unsigned int IOFrequency;  // every X cycles
    unsigned int IOResponseDate;  //date that an IO request finishes
} task;
